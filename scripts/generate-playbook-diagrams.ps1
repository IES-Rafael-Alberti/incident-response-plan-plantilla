...
foreach ($playbook in $playbooks) {
    ...
    $lines.Add("## $title")
    $lines.Add("")
    $lines.Add(("Playbook fuente: ``playbooks/{0}``" -f $playbook.Name))
    $lines.Add("")
    ...
    $lines.Add("    B --> C[""Clasificar severidad S1-S4""]")
    $lines.Add("    C --> D{""S1/S2, datos personales o activo critico?""}")
    $lines.Add("    D -- ""Si"" --> E[""Escalar a Incident Commander, Dirección, Legal y Comunicación""]")
    $lines.Add("    D -- ""No"" --> F[""Gestionar por TIC/CISO con seguimiento del IC""]")
    $lines.Add("    E --> G[""Investigar con ATT&CK: vector, alcance, evidencias e IOCs""]")
    $lines.Add("    F --> G")
    $lines.Add("    G --> H{""Compromiso confirmado?""}")
